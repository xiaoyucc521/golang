...
func NewRegister(endpoints []string, logger *log.Logger) *Register {
	return &Register{
		endpoints:   endpoints,
		dialTimeout: 3,
		logger:      logger,
...
func (r *Register) register() error {
	...
	r.leaseID = grant.ID
	r.logger.Println(fmt.Sprintf("租约ID: %v", r.leaseID))
	...
	r.logger.Println(fmt.Sprintf("名称: %v", key))
	r.logger.Println(fmt.Sprintf("存储信息: %v", string(data)))
